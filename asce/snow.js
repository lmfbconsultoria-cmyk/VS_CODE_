...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function attachEventListeners() {
        ...
        document.body.addEventListener('click', async (event) => {
            if (event.target.id === 'copy-report-btn' || event.target.id === 'copy-summary-btn') {
...
function handleRunSnowCalculation() {
    ...
    saveInputsToLocalStorage('snow-calculator-inputs', inputs);
...
function renderSnowResults(results) {
    ...
    if ((inputs.calculate_unbalanced && unbalanced.applicable) || (inputs.calculate_drift && drift.applicable)) {
        html += `<div id="snow-summary-section" class="mt-6 report-section-copyable">
                    <div class="flex justify-between items-center mb-2">
                         <h3 class="report-header flex-grow">4. Governing Load Summary (${inputs.design_method})</h3>
